...
const int fallback_voltage = 15000;
...
void sink_callback(callback_event event) {
    ...
    if (event == callback_event::source_caps_changed) {
        ...
        if (res == -1) {
            // desired voltage is not available, try fallback voltage
            res = power_sink.request_power(fallback_voltage);
        }
...
int main() {
    ...
    while (true) {
        ...
        hal.set_led(power_sink.active_voltage == desired_voltage    ? color::green
                    : power_sink.active_voltage == fallback_voltage ? color::yellow
                                                                    : color::red);
    }
}